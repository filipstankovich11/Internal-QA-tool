  ...
  return (
    <button
      ...
      style={{
        ...
        background: checked ? '#FF9780' : 'rgba(255,255,255,0.1)',
        position: 'relative',
        border: 'none',
        cursor: 'pointer',
...
function ProfileTab() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <label className="text-xs font-medium block mb-1.5" style={{ color: '#888' }}>Display name</label>
        <input
          value={name}
      ...
      <div>
        <label className="text-xs font-medium block mb-1.5" style={{ color: '#888' }}>Email</label>
        <p className="text-sm px-4 py-2.5 rounded-xl" style={{ color: '#666', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
          {user?.email}
        </p>
      </div>
      <button
    ...
    <div className="flex flex-col gap-5">
      <div className="rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <p className="text-xs mb-4" style={{ color: '#666' }}>
          We'll send a password reset link to <span style={{ color: '#aaa' }}>{user?.email}</span>
        </p>
        {sent ? (
          <p className="text-sm" style={{ color: '#10b981' }}>✓ Reset email sent — check your inbox</p>
        ) : (
          <button
            onClick={send}
            disabled={sending}
            className="text-sm px-4 py-2 rounded-xl font-medium transition-all"
            style={{ background: 'rgba(255,151,128,0.1)', color: '#FF9780', border: '1px solid rgba(255,151,128,0.2)', opacity: sending ? 0.5 : 1 }}
            onMouseEnter={e => { if (!sending) e.currentTarget.style.background = 'rgba(255,151,128,0.18)' }}
...
function PreferencesTab({ agentRecord }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <label className="text-xs font-medium block mb-1.5" style={{ color: '#888' }}>Goal score</label>
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <span className="text-xs" style={{ color: '#555' }}>out of 100</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between py-1">
        <div>
          ...
          <p className="text-xs mt-0.5" style={{ color: '#666' }}>Receive a DM when a reviewer scores your ticket</p>
        </div>
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: '#666' }}>
              ...
              <span className="ml-2 capitalize" style={{ color: '#FF9780' }}>{role}</span>
            </p>
          </div>
          <button onClick={onClose}
            className="text-2xl leading-none transition-colors" style={{ color: '#555' }}
        ...
        <div className="flex" style={{ minHeight: 300 }}>
          {/* Side tabs */}
          <div className="flex flex-col gap-0.5 p-3 shrink-0" style={{ width: 156, borderRight: '1px solid rgba(255,255,255,0.07)', background: 'rgba(0,0,0,0.15)' }}>
            {visibleTabs.map(t => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
                ...
                style={{
                  color: tab === t.id ? '#FF9780' : '#666',
                  background: tab === t.id ? 'rgba(255,151,128,0.1)' : 'transparent',
